...
def convert_coco_to_yolo(coco_json, images_dir, out_dir, copy_images=True, respect_subfolders=False):
    ...
    for img_id, info in tqdm(img_info.items(), desc="Converting"):
        ...
        if respect_subfolders:
            dst = os.path.join(out_dir, "images", file_name)
            label_path = os.path.join(
                out_dir, "labels", os.path.splitext(file_name)[0] + ".txt"
            )
        else:
...
def main():
    ...
    ap.add_argument("--respect-subfolders", action="store_true",
                    help="Preserve original subfolder structure from images_dir")
    args = ap.parse_args()
    ...
    convert_coco_to_yolo(
        ...
        respect_subfolders=args.respect_subfolders
    )
